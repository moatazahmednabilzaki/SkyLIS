...
dotnet test                   # 98 unit + architecture tests (plus a 146-check E2E in scripts/e2e.ps1)
```
...
|---|---|---|
| `admin-portal` | http://localhost:4201 | Platform console (dark theme): tenant directory with per-tenant usage meters against the plan quota (FR-SYS-011/P01.3), lifecycle actions (activate / suspend / resume / offboard — suspended tenants cannot sign in), plan changes, and the read-only tenant user monitor (P01.5); provisioning with the initial Tenant Admin account and plan validation; the plan builder (P01.3/§8 — seat and branch quotas enforced at consumption); country packs (P01.4 — seed each new tenant's taxonomy, FR-TEN-040); master data packs (P01.7 — platform test catalogue with push-to-all-tenants, FR-MDM-071); platform health (outbox pipeline status) |
| `client-portal` | http://localhost:4300 | Tenant app: credential sign-in, dashboard, global search (Ctrl+K, FR-SYS-008), patient search/registration with the duplicate merge console (P04.4), Patient 360 (P04.3) with cumulative result trends (P10.3), result amendments (P09.5), and data-subject export/erasure (P04.5); visit-registration wizard with branch selection, a real test picker, and panels/profiles at bundle prices (P03.5); visit details with sample collect/receive/reject (coded vocabulary enforced when configured), add-on tests with supplementary invoices (P05.4), cancellation with automatic credit notes, and attachments (FR-SYS-007); Branches & Departments (P03.2 — per-branch number series); Lab Setup wizard with tenant settings and catalog CSV import/export (P03.1/FR-SYS-004/FR-SYS-009); the merged M08 Reception & Phlebotomist worklists; payment capture with discounts/refunds and the Cashier & Day Close Z-report (P17.1/P17.2); M09 — results entry workbench, technical/medical validation with e-signature, critical-values console; M10 — reporting worklist with interim/final/amended rendering (tenant-branded footer), delivery, and public hash verification; M23 — live executive dashboard plus TAT/financial/quality analysis (P23.2–P23.4); the FR-SYS-001 audit-trail explorer with hash-chain integrity verification; users & roles with lock/unlock/deactivate, password reset, and self-service password change (§4.3); all live pages refresh in real time over SignalR (FR-SYS-010) |
