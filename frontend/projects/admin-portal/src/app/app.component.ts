...
@Component({
  ...
  template: `
    @if (auth.isAuthenticated()) {
      <div class="topbar">
        ...
        <span class="ctx">National Technology · Platform Console</span>
      ...
      <nav class="sidebar">
        ...
        <a routerLink="/master-data" routerLinkActive="on">🗂️ Master Data Packs</a>
        <a routerLink="/plans" routerLinkActive="on">💠 Plans</a>
        <div class="foot">console.skylis.app · v0.5</div>
      </nav>
  ...
  styles: `
    ...
    .wm { font-size: 19px; font-weight: 700; letter-spacing: -.02em; color: #fff; }
    .lis {
      font-size: 9px; font-weight: 600; letter-spacing: .18em; color: var(--accent);
      border: 1px solid #2c4a63; border-radius: 4px; padding: 1px 5px;
    }
    .portal {
      font-size: 9px; font-weight: 700; letter-spacing: .14em; color: var(--amber);
      border: 1px solid rgba(251, 191, 36, .4); border-radius: 4px; padding: 1px 6px; margin-left: 6px;
    }
    .ctx { font-size: 11px; color: var(--slate); }
    .spacer { flex: 1; }
    .sidebar {
      position: fixed; top: 56px; left: 0; bottom: 0; width: 208px;
      background: #0a1420; padding: 14px 0; display: flex; flex-direction: column;
    }
    .group { font-size: 9.5px; font-weight: 700; letter-spacing: .14em; color: #43596f; padding: 14px 18px 6px; }
    .sidebar a, .sidebar .soon { display: block; padding: 9px 18px; font-size: 12.5px; }
    .sidebar a { color: #c6d4e2; text-decoration: none; border-left: 3px solid transparent; }
    ...
    .soon { color: #43596f; }
    .soon em { font-size: 9px; font-style: normal; letter-spacing: .06em; margin-left: 6px; }
    .foot { margin-top: auto; padding: 16px 18px; font-size: 10px; color: #43596f; border-top: 1px solid var(--line); }
